...
public class WordGame : MonoBehaviour {
		...
		case GameMode.inLevel:
			...
			foreach (char cIt in Input.inputString){
				...
				if (c == ' ') {
					...
					bigLeters = ShuffleLetters(bigLetters);
					ArrangeBigLetters();
				}
			}
			break;
	...
	pubic void CheckWord(){
		...
		for (int i=0; i<currLevel.subWords.Count; i++) {
			...
			if (string.Equals (foundTestWord, subWord)) {
				HighlightWyrd (i);
				Score( wyrds[i], 1);
				foundTestWord = true;
			} else if (foundTestWord.Contains (subWord)) {
	...
	public IEnumerator CheckWord(){
		...
		if (foundTestWord) {
			...
			for (int i=0; i<containedWords.Count; i++) {
				...
				Scoreboard( wyrds[ containedWords[ndx] ], i+2); 
			}
		}
	...
	void HighlightWyrd(int ndx){
		...
		wyrds [ndx].visable = true;
	}

	void ClearBigLettersActive(){
		testWord = "";
		foreach (Letter 1 int bigLettersActive){
			bigLetters.Add(1);
			1.color = bigColorDim;
		}
